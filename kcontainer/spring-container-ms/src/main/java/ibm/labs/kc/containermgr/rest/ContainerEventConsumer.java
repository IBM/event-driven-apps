...
public class ContainerEventConsumer {
	private static final Logger LOG = Logger.getLogger(ContainerEventConsumer.class.toString());
    ...
    public ResponseEntity<String> onEvent(@Valid @RequestBody String containerEvent) {
		LOG.info("Got event: "+containerEvent);
		LOG.info("BPM service for container anomaly events at: " + bpm_anomaly_url);
		if (containerEvent.contains(ContainerEvent.CONTAINER_ADDED)) {
			// --------------------------------------------
			// ContainerAdded events
			//---------------------------------------------
			LOG.info("Received create new container event: " + containerEvent);
			ContainerCreationEvent cce = parser.fromJson(containerEvent, ContainerCreationEvent.class);
		...
		else if (containerEvent.contains(ContainerEvent.CONTAINER_ANOMALY)) {
			// --------------------------------------------
			// ContainerAnomaly events
			//---------------------------------------------
			LOG.info("Received new container anomaly event: " + containerEvent);
			// Get the ContainerAnomalyEvent objet from the event received
			ContainerAnomalyEvent cae = parser.fromJson(containerEvent, ContainerAnomalyEvent.class);
		...
		else if (containerEvent.contains(ContainerEvent.CONTAINER_ON_MAINTENANCE)) {
			// --------------------------------------------
			// ContainerOnMaintenance event
			//---------------------------------------------
			LOG.info("Received new ContainerOnMaintenance event: " + containerEvent);
			// Get the ContainerOnMaintenanceEvent objet from the event received
			ContainerOnMaintenanceEvent conm = parser.fromJson(containerEvent, ContainerOnMaintenanceEvent.class);
		...
		else if (containerEvent.contains(ContainerEvent.CONTAINER_OFF_MAINTENANCE)) {
			// --------------------------------------------
			// ContainerOffMaintenance event
			//---------------------------------------------
			LOG.info("Received new ContainerOffMaintenance event: " + containerEvent);
			// Get the ContainerAnomalyEvent objet from the event received
			ContainerOffMaintenanceEvent coffm = parser.fromJson(containerEvent, ContainerOffMaintenanceEvent.class);
		...
		else if (containerEvent.contains(ContainerEvent.CONTAINER_ORDER_ASSIGNED)) {
			// --------------------------------------------
			...
			// Others
			//---------------------------------------------
			LOG.warning("[WARNING] -  Received unexpected event: " + containerEvent);
		}
