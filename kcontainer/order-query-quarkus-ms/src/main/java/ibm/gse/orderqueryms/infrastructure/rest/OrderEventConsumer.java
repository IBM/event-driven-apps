...
public class OrderEventConsumer implements EventListener {
	private static final Logger logger = LoggerFactory.getLogger(OrderEventConsumer.class.getName());
	private final OrderDAO orderRepository = AppRegistry.getInstance().orderRepository();
	...
	public Response consumeEvent(String orderEvent) {
		...
		OrderEvent event = OrderEvent.deserialize(orderEvent);
